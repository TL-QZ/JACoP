...
class QCNetAgentEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if dataset == 'argoverse_v2':
            ...
            input_dim_r_pl2a = 3
            input_dim_r_a2a = 3
        elif dataset == 'eth_ucy' or dataset == 'sdd':
            input_dim_x_a = 4
            input_dim_r_t = 4
            # input_dim_r_pl2a = 3
            input_dim_r_a2a = 3
        ...
        if dataset == 'argoverse_v2':
            ...
            self.type_a_emb = nn.Embedding(1, hidden_dim)
        else:
            raise ValueError('{} is not a valid dataset'.format(dataset))
    ...
    def forward(self,
                ...
                residual:bool=True) -> Dict[str, torch.Tensor]:
        """
        residual: bool, whether to use residual connection for social attention
            if not, this means the output embedding will be from the aggregation of neighbors only
        """
        ...
        elif self.dataset == 'eth_ucy' or self.dataset == 'sdd':
            vel = data['agent']['velocity'][:, :self.num_historical_steps, :self.input_dim].contiguous()
            length = width = height = None
            categorical_embs = [
                self.type_a_emb(data['agent']['type'].long()).repeat_interleave(repeats=self.num_historical_steps,
        ...
        if self.dataset == 'argoverse_v2': #agent to map 
        ...
        for i in range(self.num_layers):
            ...
            if map_enc != {}:
                x_a = self.pl2a_attn_layers[i]((map_enc['x_pl'].transpose(0, 1).reshape(-1, self.hidden_dim), x_a), r_pl2a,
                                            edge_index_pl2a)
            x_a = self.a2a_attn_layers[i](x_a, r_a2a, edge_index_a2a, residual=residual)
