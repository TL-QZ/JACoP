...
from metrics.min_jfde import minJFDE
...
class minJADE(Metric):
    ...
    def update(self,
        ...
        valid_cnt = valid_mask.sum(dim=-1).clamp_min(1.0)
        if min_criterion == 'JFDE':
            ...
            inds_best = torch.norm(
                ...
                target[torch.arange(pred.size(0)), inds_last].unsqueeze(-2), p=2, dim=-1).sum(dim=0).argmin()

            per_agent = (
                (torch.norm(pred_topk[torch.arange(pred.size(0)), inds_best] - target, p=2, dim=-1) * valid_mask)
                .sum(dim=-1) / valid_cnt
            )
        ...
        elif min_criterion == 'JADE':
            per_mode = (
                (torch.norm(pred_topk - target.unsqueeze(1), p=2, dim=-1) * valid_mask.unsqueeze(1))
                .sum(dim=-1) / valid_cnt.unsqueeze(-1)
            )
            batch_sum = per_mode.sum(dim=0).min()
            batch_count = pred.size(0)
            batch_mean = batch_sum / max(batch_count, 1)
            per_agent = per_mode.mean(dim=-1)
        else:
